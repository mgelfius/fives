<script lang="ts">
	...
	function update(event: MouseEvent) {
		...
		const keyElement = document.querySelector(`[data-key="${key}" i]`);
		if (key === "backspace") {
			if (!isNaN(currentGuess.slice(-1)) && currentGuess.length > 0) {
				numberArray = [...numberArray, currentGuess.slice(-1)];
			}
...
<svelte:head>
	<title>Fives</title>
	<meta name="description" content="Fives" />
</svelte:head>
...
<style>
	...
	.successful-guess .successful-guess {
		border: none;
		background-color: #d4f0d4;
	}
